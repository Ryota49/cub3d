...
{
	if (ft_atoi(parsing->splitter[i]) >= 0
		&& ft_atoi(parsing->splitter[i]) <= 255)
	{
...
void	split_values(t_utils_parsing *parsing)
{
	...
	if (parsing->splitter[1][0] == '\0' || parsing->splitter[2][0] == '\0'
		|| parsing->splitter[3][0] == '\0')
